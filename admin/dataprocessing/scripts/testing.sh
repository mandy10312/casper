...
set file="../phase1/2010-SP-0725.txt"
...
head -$firstcut $file | tail -$students | sed -e 's+\*\*\*-\*\*-+  +' -e 's+   *+,+g' -e 's+ +_+g' -e 's+$+,NG+' -e "s+\([^,]\)\([^,]*\),\([^,]\)\([^,]*\),\([^,]\)\([^,]*\),\([^,][^,]*\),\([^,][^,]*\),\([^,][^,]*\)+$year,""$semester"",""$dept"",""$course"",""$section"",""$regcode"",""$coursename"",""$instructor"",""\1\3"",""\5\6"",""\1\2"",""\3\4"",""\7"",""\9""+" -e 's+,NG$++' 
